...
class Utils
{
  ...
  static dist2(v, w) { return Utils.sqr(v.x - w.x) + Utils.sqr(v.y - w.y) }
  ...
  static distToSegment(p, v, w) { return Math.sqrt(Utils.distToSegmentSquared(p, v, w)); }

  static pointInSegment(p, v, w, maxDist){
    return Utils.distToSegment(p,v,w) <= maxDist;
  }

  // THANK_YOU: http://stackoverflow.com/questions/2259476/rotating-a-point-about-another-point-2d
  static rotatePoint(pivot, p, angle){
    ...
    var newX = result.x * c - result.y * s;
    var newY = result.x * s + result.y * c;
